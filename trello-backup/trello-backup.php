...
//print_r($k["cards"]);
$card_shortLink = array();
foreach($k["cards"] as $card){
  $c = get_object_vars($card);
  //print_r($c["idShort"]." ".$c["shortLink"]);
  //echo "</br>-----------------</br>";
  $card_shortLink[$c["idShort"]] = $c["shortLink"];
}


// 5) Backup now! Fetch all Trello Cards Actions, ... (attachs, links, ...)
foreach($boards as $id => $board) {

  if ($board->name == "~4050") {

    foreach($card_shortLink as $idShort => $shortLink) {
      echo $idShort." - ".$shortLink."\n";

      $url_individual_card_json = "https://api.trello.com/1/cards/$shortLink/actions?key=$key&token=$application_token";

      $filename = "$path/card-$idShort.json";

      echo "recording https://api.trello.com/1/cards/$shortLink/actions?key=$key&token=$application_token in filename $filename...\n";
